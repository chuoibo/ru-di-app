...
`docs/archive/claude/2026-09-10/motion/do-motion.sh` force-stop/reset trước helper còn launch/onboard/logout. README lại nói warm xong mới reset. B đã chạy canary riêng, không chạm máy ảo: cả bốn Maestro cố ý exit 42, số khung là `?`, nhưng wrapper vẫn **exit 0** vì chỉ ghi rc vào bảng rồi kết thúc bằng `cat`.
