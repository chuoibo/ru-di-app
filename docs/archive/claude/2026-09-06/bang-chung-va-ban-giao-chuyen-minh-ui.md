...
Báo cáo `docs/archive/codex/2026-09-06/bao-cao-chuyen-minh-ui-rudi.md` chẩn đoán đúng: bộ da v2 (màu, chữ, con dấu) đã có, nhưng **bố cục vẫn là dashboard v1**: thẻ lồng thẻ, ô thống kê, banner AI, và số liệu màn tự bịa. Người dùng nhìn thấy một bảng điều khiển chứ không thấy việc mình cần làm. Cách giải, áp cho từng màn:
...
| Bảng live ở cỡ chữ 1.3 / expanded | Expanded: mini-bảng 22→24→25→27→28 ở 1800×2400@320 (hàng trên). Cỡ chữ 1.3 (`font_scale 1.3`, 360dp): bảng live đầy đủ 21 flow `--anh` chạy ba lượt trong ngày — lượt một 17/21 (27 «Tạo kèo» dưới nếp gấp sau bàn phím → nút thành `footer`; 28/32 đỏ dây chuyền vì không có kèo; 38 kẹt ở «Hội An» của flow 35 → flow trả điểm đến về Đà Lạt), lượt ba (16:27, `5301d3a9`) **20/21** — chỉ 38 đỏ ở câu «Người ngoài nhóm không thấy» nằm dưới nếp gấp sau dải ảnh → câu lên dưới tiêu đề mục, flow cuộn tới chú thích | mini-bảng 22→24→25→26→35→38 ở 1.3 trên `be0df3c7` (17:13): **XANH** — 38 qua sau khi 35 để máy ở Hội An, 97 thẻ có ảnh, 0 thiếu credit, canary OTP đỏ đúng chỗ. Bảng cuối trên commit chốt `bf7049ec` (17:38, cỡ chữ 1.3, `--anh`): **21/21 flow xanh**; harness dừng đỏ ở phép kiểm máy chủ sau flow 32 «có 2 kỷ niệm, mong 1» — kỷ niệm thứ hai là chính tấm ảnh harness đặt cho flow 38 vào «Hoi QA» (hệ quả của vá 4g), không phải app; phép kiểm đổi sang chỉ đếm kỷ niệm check-in; mini-bảng 22→24→25→26→27→32→38 (18:19, `--anh`): **XANH** — tường «Hoi QA» «đúng một check-in» bên cạnh ảnh của flow 38, canary OTP đỏ đúng chỗ. Flow 26 cũng vá: sau khi quay lại từ chi tiết, danh sách giữ chỗ đã cuộn nên «chờ thấy» tiêu đề không cuộn được trên stack 152 nơi → cuộn lên tới tiêu đề |
| Flow 38 (ảnh có giấy phép) | stack 47597 nhập 144 địa điểm OSM Đà Lạt thật (`import_osm_places.py`) rồi 97 ảnh Commons theo geosearch 250 m (`import_place_photos.py --moi-noi 1`, 0 file bỏ vì thiếu xuất xứ) — dữ liệu thật chỉ vào DB dùng một lần, không vào Git; `/places?destination=d-da-lat` trả 152 nơi, 97 có ảnh bìa kèm tác giả + giấy phép | mini-bảng 22→24→25→26→38 `--anh` (15:24): **XANH** — thẻ Khám phá in «Ảnh quanh đây: …», màn địa điểm có ảnh bìa + credit + câu «Ảnh có giấy phép chụp quanh đây…» (thêm ở `8dc7103b`, trước đó thiếu khi chỗ chỉ có một ảnh) + «Ảnh của nhóm bạn» với chú thích và câu «Người ngoài nhóm không thấy»; máy chủ xác nhận 97 thẻ có ảnh bìa, 0 thiếu credit. Harness sửa: ảnh nhóm đặt vào nhóm của người đang cầm máy ở flow 38 (D sau flow 26), trước đó đặt vào nhóm riêng của người flow 22 nên D không bao giờ thấy |
| Ma trận §13.2: mạng lỗi / stale | Cắt wifi+data trên máy ảo KHÔNG tạo được mất mạng (app tới API qua đường hầm `adb reverse`, loopback) — đo bằng gỡ/cắm chính đường hầm (`mang-loi.sh`). Kết quả: Tin nhắn «Chưa đọc được danh sách nhóm» + Thử lại, Lên plan «Chưa đọc được kèo» + Thử lại, Khám phá khởi động lạnh «Chưa đọc được danh mục» + Thử lại; nối lại rồi mở tab là tự nạp. Lỗ hổng tìm thấy: Khám phá đang mở sẵn giữ dữ liệu cũ không có cách đọc lại → `RudiScreen onRefresh` (kéo-để-làm-mới) cho Khám phá/Tin nhắn/Lên plan; kéo xuống khi mất máy chủ ra trạng thái lỗi, nối lại rồi «Thử lại» về danh sách; pill điểm đến không «Đang đọc…» mãi khi đọc hỏng | dump + ảnh `mang-loi2/` (scratchpad) |
