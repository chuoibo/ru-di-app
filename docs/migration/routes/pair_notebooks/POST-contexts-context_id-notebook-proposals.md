...
- `POST /contexts/{context_id}/notebook/proposals`: `purpose=chia_gu`: 409 `consent_missing` ngoài «Một đôi»; trong đó tạo lời đề nghị, người đề nghị đồng ý và hoàn tất ngay (một người quyết), không bao giờ nằm ở `pending_proposals`; bật lại khi đang bật trả lời đề nghị cũ.
...
- `POST /contexts/{context_id}/notebook/proposals`: Chỉ đi qua `_pair_roster_or_404` (cùng lệnh đọc, cùng câu trả lời) hoặc bị cổng nối theo tên hàm kéo vào — hành vi không đổi.
...
- `POST /contexts/{context_id}/notebook/proposals`: Chỉ bị cổng nối theo tên hàm (`_open_paper_id`, `pair_notebook`, repository) kéo vào — hành vi không đổi.
...
- `POST /contexts/{context_id}/notebook/proposals`: Chỉ bị cổng nối theo tên hàm kéo vào — hành vi không đổi.
