...
- `POST /contexts/{context_id}/memories`: Chỉ đi qua `_pair_roster_or_404` (cùng lệnh đọc, cùng câu trả lời) hoặc bị cổng nối theo tên hàm kéo vào — hành vi không đổi.
...
- `POST /contexts/{context_id}/memories`: Chỉ bị cổng nối theo tên hàm (`_open_paper_id`, `pair_notebook`, repository) kéo vào — hành vi không đổi.
...
- `POST /contexts/{context_id}/memories`: Chỉ bị cổng nối theo tên hàm kéo vào — hành vi không đổi.
