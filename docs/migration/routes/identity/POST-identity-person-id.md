...
- **Làn limiter.** `parity/scenarios/w2/limiter/POST-identity-person-id-limit.yaml`, id `w2/limiter/post-identity-person-id-limit` (27 bước, `lane: limiter`): key idempotency rỗng, 405 và 307 không tốn lượt; 20 lượt tới handler, kể cả thân rỗng, chỉ khoảng trắng, JSON cắt cụt, chuỗi ở gốc, `NaN`, `1e999`, khoá trùng (lấy giá trị sau), BOM UTF-8, `Content-Type: text/plain`, tiền tố `084`; lượt 21 là 429 kể cả khi thân hỏng; replay một câu trả lời đã lưu vẫn 200 vì middleware trả lời trước handler; `POST /friends/lookup` vẫn đi tiếp (limiter riêng). Lăn cửa sổ giữa chừng và xoá bảng khi quá 10 000 cặp chưa được so (ADR-0029 §2.4).
...
- `json.loads` của Python chấp nhận `NaN`, `Infinity`, số rất lớn, BOM UTF-8 và khoá trùng (lấy giá trị sau); `NaN`, `1e999`, BOM và khoá trùng nằm trong kịch bản làn limiter ở trên, `Infinity` chưa có bước.
