...
`parity/scenarios/w2-storage-key-gap/stories/DELETE-stories-story_id.yaml`, id `w2-storage-key-gap/stories/delete-stories-story_id` (36 bước), được xác nhận **chỉ trên wire** (không DSN, vẫn có `--candidate-tap` và `--served-routes`):
...
- `parity/scenarios/w2-storage-key-gap/stories/prod-auth.yaml`, id `w2-storage-key-gap/stories/prod-auth`: `friend_deletes_authors_story` (403), `author_deletes_story` (204).
