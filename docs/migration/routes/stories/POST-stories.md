...
`parity/scenarios/w2/stories-photo/POST-stories.yaml`, id `w2/stories-photo/post-stories` (23 bước, lane DB):
...
- `parity/scenarios/w2/stories-photo/prod-auth.yaml`, id `w2/stories-photo/prod-auth` (16 bước): `author_creates_with_empty_roles_header` (201; dev sẽ 403), `author_creates_story` + `author_create_replay` (scope là bearer, `X-Actor-ID` gửi kèm không đổi scope), `friend_same_key_is_another_scope`.
