...
`parity/scenarios/w2-storage-key-gap/stories/GET-stories.yaml`, id `w2-storage-key-gap/stories/get-stories` (51 bước), được xác nhận **chỉ trên wire** (không DSN, vẫn có `--candidate-tap` và `--served-routes`):
...
- `parity/scenarios/w2-storage-key-gap/stories/prod-auth.yaml`, id `w2-storage-key-gap/stories/prod-auth`: `friend_feed_not_friend_yet`, `friend_feed_sees_story` (tên tác giả là `people.display_name` được seed), `author_feed_own_story`, `friend_feed_after_delete`.
