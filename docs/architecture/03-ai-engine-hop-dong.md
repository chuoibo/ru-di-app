...
| 6 | Engine S1: Nếp qua Go, chưa stream; `Engine.Run`; eval T1 trong CI | Tách hai phần; lát 6 chỉ xong khi cả hai xong. **6a (engine) xong**: `0a752a7`, cộng commit sửa theo review phản biện ngay sau `34bc428`. Luật tiền viết lại: 157/157 và 0/104 trên corpus 157+104 câu do người khác viết, nhưng tác giả luật đã đọc corpus trước khi viết nên đó không phải số giữ riêng (số giữ riêng thật của luật cũ: 51/157, 2/104). `Sink` đúng thiết kế 01 §2: engine không phát xong/thất bại. «Bây giờ» có test đỏ được. **6b (eval T1 trong CI) chưa**: `aieval`, `cmd/rudi-eval --mo-hinh kich-ban`, corpus Nếp, `scripts/eval_kich_ban.sh` và job CI (thiết kế 06). **Cờ `MOBILE_AI_ENGINE_NEP` ở `brain` tới khi T1 xanh**, ADR-0037 được ký và review bảo mật khoá trong core xong. Compose chỉ bật `go` khi ghép rõ `docker-compose.nep-go.yml`. `aiboicanh` dời sang lát 9. `make parity` chưa chạy ở máy sửa (không có Docker) |
