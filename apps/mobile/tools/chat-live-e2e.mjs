...
const pendingPlan=pages[0].waitForResponse(r=>r.url().endsWith(`/contexts/${fixture.groupId}/messages`)&&r.request().method()==='POST');
await send(pages[0],`/plan ${run} cả nhóm muốn đi cà phê Đà Lạt tối nay, hãy phác lịch trình`);
const plan=await pendingPlan,body=await plan.json();await sleep(5500);
result.steps.push({name:'slash_plan_actual',status:plan.status(),intent:body.intent,intentError:body.intent_error,companion:body.companion,uiText:await pages[0].evaluate(()=>document.body.innerText.slice(-7000))});save();
