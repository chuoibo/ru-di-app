 ...
 * §16.2). `radius.base`, because this is a sheet and not a control. No
 * shadow: elevation is declared once, and a sheet has an edge, not a drop.
 *
 * ## What it is not
 *
...
export function ToGiay({
  ...
  return (
    <View
      style={[
        styles.to,
        { backgroundColor: colors.paper, borderColor: colors.lineStrong, borderRadius: radius.base, padding: space.md },
        style,
      ]}
      testID={testID}
    >
 ...
 * across the sheet. A plain `View`, not SVG -- a crease is a line where the
 * paper was folded, and it reads as one from the three even rows around it,
 * not from any shading trick (spec §16.3: the light scheme has no colour
 * brighter than `paper` to catch light on, so the crease is the same one nét
 * in both schemes).
 */
export function VetGap({ style }: { style?: StyleProp<ViewStyle> }) {
  const { colors, space } = useRudiTheme();
  return <View style={[styles.vet, { backgroundColor: colors.paperShade, marginVertical: space.sm }]} />;
}

const styles = StyleSheet.create({
  to: { alignSelf: "stretch", borderWidth: StyleSheet.hairlineWidth, overflow: "hidden" },
  goc: { position: "absolute", top: 0, right: 0 },
