...
export type SuPhieu = { kieu: "khai"; phieu: PhieuNguCanh } | { kieu: "bo"; phieu: PhieuNguCanh };

/**
 * A screen's slip belongs to the screen that is focused, and only that screen
 * may withdraw it.
 *
 * This used to be enforced by the provider clearing the slip whenever the
 * pathname changed. That is an ordering bet it loses: expo-router mounts the
 * incoming screen, and runs its effects, before the provider sees the new
 * pathname, so the clear landed after the new screen had declared and wiped
 * it (measured 24-09 on the web build, sign-in to Khám phá: declared at
 * `/explore`, provider still on `/otp`, Nếp never saw the slip). Declaring on
 * focus and withdrawing on blur needs no pathname at all. Withdrawing removes
 * only the slip that screen declared, so a screen that blurs after the next
 * one has focused cannot take the next one's slip with it.
 */
export function ghiPhieu(cu: PhieuNguCanh | null, su: SuPhieu): PhieuNguCanh | null {
  if (su.kieu === "khai") return su.phieu;
  return cu === su.phieu ? null : cu;
}
