...
var (
	...
	// A coordinate, or a pair of them: degrees up to 180 with four to eight
	// decimal places, the whole part not starting with 0 («10.7768,
	// 106.7008»). A phone starts with 0 or 84; a run starting with 84 is
	// never read as a coordinate (khongPhaiLienLac).
	toaDo = regexp.MustCompile(`^-?(?:[1-9]\d?|1[0-7]\d|180)\.\d{4,8}(?:,\s?-?(?:[1-9]\d?|1[0-7]\d|180)\.\d{4,8})?$`)
	...
	// needs its currency after it -- a coordinate, and a leading date
	// (26.09.2026 or 26/09/2026, then maybe the hour, or a dash and a second
	// date: a range of days). An amount never starts with 0, so a phone
	// written with dots in groups of three, three and four is still a phone.
	...
	tuNhan = regexp.MustCompile(`\b(minh|nep|toi|em|to)\s+(da|vua|da vua)\s+((giup|ho|thay)\s+(ban|cau|anh|chi|em|nhom|ca nhom|moi nguoi)\s+)?` +
		`(chuyen (tien|khoan|cho|giup|ho|gium|thay|lai|\d\S*)|(ck|bank|ban|tra|gui) \d\S*|(chuyen|ck|bank|ban|tra|dua)(\s+\S+){1,2}\s+(` + soTienRa + `)|gui\s+\S+\s+(` + soTienRa + `)|ghi (no|vao so|lai khoan|khoan)|ghi lai(\s+\S+){0,3}\s+(no|tien|` + soTienRa + `)|thanh toan|tra (tien|no|giup|ho|gium|thay|cho|lai)|tao (keo|nhom|binh chon|lich|khoan|nhac)|chot (keo|lich|dia diem|quan|cho)|dat (ban|cho|lich|ve|phong|keo)|gui (tin|loi moi|tien|ma|qr|stk|so tai khoan|cho \S+)|ck|nhan tin|moi (ban|nguoi|ca nhom|moi nguoi)|them (ban|nguoi|vao (keo|nhom|lich|so|ca nhom|danh sach (keo|muon di|yeu thich)))|xoa (keo|tin|khoan|anh|nhom)|huy (keo|lich|ban|cho)|luu (lai|vao)|hen (gio|lich)|nhac (ban|moi nguoi|ca nhom|lich)|nhac \S+ (tra|chuyen|dong|gop|ck|gui)|(cap nhat|sua|doi) (keo|lich|khoan))\b`)
	...
	tuNhanRoi = regexp.MustCompile(`\b(minh|nep|toi|em|to)\s+(chuyen|ck|bank|tra|ban|dua)(\s+cho)?(\s+\S+){0,2}\s+(` + soTienRa + `)(\s+\S+){0,2}\s+roi\b` +
...
func boTraLoi(g string) string {
	...
	for _, m := range traLoiGui.FindAllStringIndex(g, -1) {
		...
		if (!traLoiNoi.MatchString(g[m[0]:m[1]]) && tienDauTraLoi.MatchString(duoi)) || tienSauTraLoi.MatchString(duoi) {
			continue
		}
...
func khongPhaiLienLac(run, sau string) bool {
	...
	if toaDo.MatchString(run) && !strings.HasPrefix(strings.TrimPrefix(run, "-"), "84") {
		return true
	}
...
func soLienLac(text string) LoaiRa {
	text = ngoacSo.ReplaceAllString(text, "$1")
...
func (d DauRa) Kiem(text string) LoaiRa {
	...
	g := Gap(text)
	if c := boTraLoi(g); tuNhan.MatchString(c) || tuNhanDauCau.MatchString(c) || tuNhanRoi.MatchString(c) {
		return RaTuNhan
	}
