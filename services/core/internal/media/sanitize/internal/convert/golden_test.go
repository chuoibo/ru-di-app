...
func writeGolden(t *testing.T, answers map[string]string) {
	...
	data, err := json.MarshalIndent(goldenFile{
		Note:  "Pillow 12.2.0 answers for convert corpus() cases, regenerated from seeds; update with MEDIA_GOLDEN_UPDATE=1 go test -tags oracle",
		Cases: picked,
	}, "", "  ")
	if err != nil {
		t.Fatal(err)
...
func TestGolden(t *testing.T) {
	...
	for name, want := range golden.Cases {
		...
		if got != want {
			t.Errorf("%s: got %s, Pillow %s (%v)", name, got, want, err)
		}
	}
}
