...
docker run --rm --network host \
  ...
  # -timeout 30m dưới kia: mặc định của `go test` là 10 phút MỖI package, và
  # internal/repo vượt nó trên CI -- "FAIL mobile/services/core/internal/repo
  # 600.011s", không ca nào đỏ, chỉ hết giờ. Tầng này là một oracle cho mỗi
  # method repository trên PostgreSQL thật và lớn lên theo từng sóng.
  #
  # Comment phải nằm Ở ĐÂY, không nằm giữa chuỗi gán bên dưới: một dòng `#` sau
  # dấu `\` biến cả chuỗi `VAR=... \` thành biến shell KHÔNG export, `go test`
