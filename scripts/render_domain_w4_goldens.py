...
def expense_dict(
    participants, total, items=(), surcharges=(), discounts=(), advancer=None
) -> dict:
    return {
...
def allocator_edges() -> list[dict]:
    ...
    for k, total in enumerate(totals):
        for advancer in ("a", "c", None, "z", ""):
            out.append(
                allocate_case(
                    f"even/{k}/{advancer}",
                    expense_dict(three, total, advancer=advancer),
                )
            )
    for count in (1, 2, 7, 40):
        ...
        for total in (count - 1, count + 1, MAX):
            out.append(
                allocate_case(
                    f"many/{count}/{total % 1000}",
                    expense_dict(people, total, advancer=people[-1]),
                )
            )
    ...
    for advancer in [None, "outsider"] + ranking_ids:
        out.append(
            allocate_case(
                f"rank/even/{advancer}",
                expense_dict(ranking_ids, 13, advancer=advancer),
            )
        )
        out.append(
            allocate_case(
                ...
                expense_dict(
                    ...
                    31,
                    [
                        item("i1", 10, ranking_ids[:3]),
                        item("i2", 20, ranking_ids[2:]),
                        item("i3", 1, ranking_ids[::2]),
                    ],
                    advancer=advancer,
                ),
            )
    ...
    out += [
        ...
        allocate_case(
            "zero_item_named_total", expense_dict(three, 0, [item("total", 0, three)])
        ),
        allocate_case(
            "zero_item_named_total_with_another",
            expense_dict(three, 7, [item("total", 0, ["a"]), item("i", 7, three)]),
        ),
        allocate_case(
            "zero_surcharge_named_total",
            expense_dict(
                three, 5, [item("i", 5, three)], [surcharge("total", "fee", 0, "even")]
            ),
        ),
        allocate_case(
            "zero_proportional_surcharge_named_total",
            expense_dict(
                three,
                5,
                [item("i", 5, ["a"])],
                [surcharge("total", "vat", 0, "proportional")],
            ),
        ),
        allocate_case(
            "zero_item_discount_named_total",
            expense_dict(
                three, 5, [item("i", 5, three)], [], [discount("total", 0, "item", "i")]
            ),
        ),
        allocate_case(
            "zero_global_discount_named_total",
            expense_dict(
                three,
                5,
                [item("i", 5, three)],
                [],
                [discount("total", 0, "global_proportional")],
            ),
        ),
        allocate_case(
            "zero_named_Total_is_refused",
            expense_dict(three, 0, [item("Total", 0, three)]),
        ),
        allocate_case(
            "total_zero_item_one_mismatch",
            expense_dict(three, 0, [item("i", 1, three)]),
        ),
    ]
    ...
    for k, amount in enumerate(BAD_AMOUNTS):
        out.append(
            allocate_case(
                f"cap/item/{k}", expense_dict(three, 5, [item("i", amount, three)])
            )
        )
        out.append(
            allocate_case(
        ...
        out.append(
            allocate_case(
                ...
                expense_dict(
                    ...
                    [item("i", 5, three)],
                    [],
                    [discount("d", amount, "item", "i")],
                ),
            )
        )
        out.append(
            allocate_case(
                f"cap/total_itemized/{k}",
                expense_dict(three, amount, [item("i", 5, three)]),
            )
        )
    out += [
        allocate_case(
            "cap/max_item_exact", expense_dict(three, MAX, [item("i", MAX, three)])
        ),
        allocate_case(
            "cap/two_max_items_total_past_max",
            expense_dict(
                three, 2 * MAX, [item("i", MAX, three), item("j", MAX, ["a"])]
            ),
        ),
        allocate_case(
            "cap/two_max_items_total_max",
            expense_dict(three, MAX, [item("i", MAX, three), item("j", MAX, ["a"])]),
        ),
        allocate_case(
            "cap/max_even_seven",
            expense_dict([f"q{i}" for i in range(7)], MAX, advancer="q3"),
        ),
        allocate_case(
            "precedence/negative_beats_zero",
            expense_dict(three, 5, [item("i", 0, three), item("j", -1, three)]),
        ),
        allocate_case(
            "precedence/zero_beats_too_large",
            expense_dict(three, 5, [item("i", MAX + 1, three), item("j", 0, three)]),
        ),
        allocate_case(
            "precedence/too_large_beats_kind",
            expense_dict(
                three, 5, [item("i", 2**64, three)], [surcharge("s", "", 1, "even")]
            ),
        ),
        allocate_case(
            "precedence/negative_past_int64_beats_too_large",
            expense_dict(three, 2**70, [item("i", -(2**64), three)]),
        ),
        allocate_case(
            "precedence/mode_beats_scope",
            expense_dict(
                three,
                5,
                [item("i", 5, three)],
                [surcharge("s", "fee", 1, "Even")],
                [discount("d", 1, "per_item", "i")],
            ),
        ),
        allocate_case(
            "precedence/scope_beats_mismatch",
            expense_dict(
                three,
                5,
                [item("i", 5, three)],
                ...
                [
                    discount("d", 1, "bogus"),
                    discount("e", 1, "global_proportional", "i"),
                ],
            ),
        ),
        allocate_case(
            "precedence/mismatch_beats_empty_shared_by",
            expense_dict(three, 5, [item("i", 5, [])], [], [discount("d", 1, "item")]),
        ),
        allocate_case(
            "precedence/empty_beats_duplicate_shared_by",
            expense_dict(three, 5, [item("i", 5, ["a", "a"]), item("j", 5, [])]),
        ),
        allocate_case(
            "precedence/duplicate_shared_by_beats_unknown",
            expense_dict(three, 5, [item("i", 5, ["z"]), item("j", 5, ["a", "a"])]),
        ),
        allocate_case(
            "precedence/unknown_participant_beats_unknown_item",
            expense_dict(
                three, 5, [item("i", 5, ["z"])], [], [discount("d", 1, "item", "nope")]
            ),
        ),
        allocate_case(
            "precedence/unknown_item_beats_exceeds_item",
            expense_dict(
                three,
                5,
                [item("i", 5, three)],
                [],
                [discount("d", 9, "item", "i"), discount("e", 1, "item", "nope")],
            ),
        ),
        allocate_case(
            "precedence/exceeds_item_beats_exceeds_base",
            expense_dict(
                three,
                0,
                [item("i", 5, three)],
                ...
                [
                    discount("d", 6, "item", "i"),
                    discount("g", 100, "global_proportional"),
                ],
            ),
        ),
        allocate_case(
            "precedence/exceeds_base_beats_reconciliation",
            expense_dict(
                three,
                999,
                [item("i", 5, three)],
                [],
                [discount("g", 6, "global_proportional")],
            ),
        ),
        allocate_case(
            "precedence/invalid_participant_beats_no_duplicate",
            expense_dict([" a", " a"], 5),
        ),
        allocate_case(
            "precedence/duplicate_participant_beats_entity",
            expense_dict(["a", "a"], 5, [item("", 5, ["a"])]),
        ),
        allocate_case(
            "precedence/entity_beats_duplicate_entity",
            expense_dict(
                three,
                5,
                [item("i", 5, three), item("i", 5, three)],
                [surcharge(" s", "fee", 1, "even")],
            ),
        ),
        allocate_case(
            "duplicate_surcharge_id",
            expense_dict(
                three,
                7,
                [item("i", 5, three)],
                [surcharge("s", "fee", 1, "even"), surcharge("s", "vat", 1, "even")],
            ),
        ),
        allocate_case(
            "namespaces_may_share_an_id",
            expense_dict(
                three,
                9,
                [item("x", 5, three)],
                [surcharge("x", "fee", 5, "even")],
                [discount("x", 1, "item", "x")],
            ),
        ),
    ]
    ...
    for k, bad in enumerate(
        ...
        out.append(
            allocate_case(f"id/item/{k}", expense_dict(three, 5, [item(bad, 5, three)]))
        )
    for k, good in enumerate(
        (
            ...
            "a b",
            "x" * 64,
            E_DOT_CIRCUMFLEX * 21 + "a",
            GRINNING_FACE * 16,
        )
    ):
        out.append(
            allocate_case(
                f"id/good/{k}", expense_dict([good, "b"], 10, [item(good, 10, [good])])
            )
        )
    for k, long in enumerate(
        ("x" * 65, E_DOT_CIRCUMFLEX * 21 + "ab", GRINNING_FACE * 16 + "a")
    ):
        out.append(
            allocate_case(f"id/long_participant/{k}", expense_dict([long, "b"], 10))
        )
        out.append(
            allocate_case(
                f"id/long_discount/{k}",
                expense_dict(
                    three,
                    4,
                    [item("i", 5, three)],
                    [],
                    [discount(long, 1, "global_proportional")],
                ),
            )
        )
    ...
    for k, kind in enumerate(
        ...
        out.append(
            allocate_case(
                f"kind/{k}",
                expense_dict(
                    three, 6, [item("i", 5, three)], [surcharge("s", kind, 1, "even")]
                ),
            )
        )
    for k, mode in enumerate(
        ("even", "proportional", "Even", "even ", "", "split_evenly")
    ):
        out.append(
            allocate_case(
                f"mode/{k}",
                expense_dict(
                    three, 6, [item("i", 5, ["a"])], [surcharge("s", "fee", 1, mode)]
                ),
            )
        )
    for k, scope in enumerate(("item", "global_proportional", "Item", "", "per_item")):
        out.append(
            allocate_case(
                f"scope/{k}/target",
                expense_dict(
                    three, 4, [item("i", 5, three)], [], [discount("d", 1, scope, "i")]
                ),
            )
        )
        out.append(
            allocate_case(
                f"scope/{k}/none",
                expense_dict(
                    three, 4, [item("i", 5, three)], [], [discount("d", 1, scope)]
                ),
            )
        )
    ...
    out += [
        allocate_case(
            "fallback/item_fully_discounted",
            expense_dict(
                three,
                4,
                [item("i", 5, ["a"])],
                [
                    surcharge("s", "fee", 3, "proportional"),
                    surcharge("t", "fee", 1, "proportional"),
                ],
                [discount("d", 5, "item", "i")],
            ),
        ),
        allocate_case(
            "fallback/global_discount_equals_base",
            expense_dict(
                three,
                7,
                [item("i", 5, ["a", "b"])],
                [surcharge("s", "fee", 7, "proportional")],
                [discount("g", 5, "global_proportional")],
            ),
        ),
        allocate_case(
            "fallback/twice_warns_once",
            expense_dict(
                three,
                8,
                [item("i", 5, ["a"])],
                [
                    surcharge("s", "fee", 4, "proportional"),
                    surcharge("t", "vat", 4, "proportional"),
                ],
                [discount("g", 5, "global_proportional")],
            ),
        ),
        allocate_case(
            "even_surcharge_with_zero_basis_no_warning",
            expense_dict(
                three,
                4,
                [item("i", 5, ["a"])],
                [surcharge("s", "fee", 4, "even")],
                [discount("g", 5, "global_proportional")],
            ),
        ),
        allocate_case(
            "global_discount_exactly_base_zero_total",
            expense_dict(
                three,
                0,
                [item("i", 5, three)],
        ...
        allocate_case(
            "item_discount_equals_item",
            expense_dict(
                three,
                5,
                [item("i", 5, ["a"]), item("j", 5, ["b"])],
                [],
                [discount("d", 5, "item", "i")],
            ),
        ),
        allocate_case(
            "two_item_discounts_exceed_together",
            expense_dict(
                three,
                -1,
                [item("i", 5, ["a"])],
                [],
                [discount("d", 3, "item", "i"), discount("e", 3, "item", "i")],
            ),
        ),
        allocate_case(
            "zero_share_warning_with_outside_advancer",
            expense_dict(three, 9, [item("i", 9, ["a", "b"])], advancer="zz"),
        ),
        allocate_case(
            "zero_share_no_warning_at_total_zero",
            expense_dict(
                three,
                0,
                [item("i", 5, ["a"])],
        ...
        allocate_case(
            ...
            expense_dict(
                three,
                101,
                [item("i", 1, ["a"]), item("j", 2, ["b"]), item("k", 4, ["c"])],
                [surcharge("s", "fee", 94, "proportional")],
            ),
        ),
        allocate_case(
            "big_numerators",
            expense_dict(
                ...
                MAX,
                [
                    item("i", MAX - 12345, ["a", "b", "c"]),
                    item("j", 999_983, ["d", "e", "f", "g"]),
                ],
                [surcharge("s", "fee", 12345, "proportional")],
                [discount("d", 999_983, "global_proportional")],
            ),
        ),
        allocate_case(
            "advancer_empty_string_in_participants", expense_dict(["", "b"], 3)
        ),
        allocate_case(
            "advancer_empty_string_outside", expense_dict(three, 4, advancer="")
        ),
        allocate_case("no_participants_total_negative", expense_dict([], -5)),
        allocate_case(
            "no_participants_with_items", expense_dict([], 5, [item("i", 5, ["a"])])
        ),
    ]
    return out
...
def fuzz_valid_expense(rng: random.Random) -> dict:
    count = (
        rng.choice((1, 2, 2, 3, 3, 3, 4, 4, 5, 6, 8))
        if rng.random() < 0.97
        else rng.randint(9, 20)
    )
    participants = rng.sample(PARTICIPANT_POOL, count)
    if rng.random() < 0.18:
        total = rng.choice((0, 1, count - 1, count, count + 1, fuzz_amount(rng), MAX))
        return expense_dict(
            participants, total, advancer=fuzz_advancer(rng, participants)
        )

    ids = rng.sample(ENTITY_POOL, rng.choice((1, 1, 2, 2, 3, 4, 5)))
    items = [
        item(item_id, fuzz_amount(rng), rng.sample(participants, rng.randint(1, count)))
        for item_id in ids
    ]
    discounts = []
    ...
    for _ in range(rng.choice((0, 0, 1, 2, 3))):
        ...
        headroom = target["amount_vnd"] - sum(
            d["amount_vnd"] for d in discounts if d["item_id"] == target["item_id"]
        )
        if headroom <= 0:
            continue
        amount = headroom if rng.random() < 0.15 else rng.randint(1, headroom)
        discounts.append(
            discount(next(discount_ids), amount, "item", target["item_id"])
        )
    base = sum(i["amount_vnd"] for i in items) - sum(d["amount_vnd"] for d in discounts)
    for _ in range(rng.choice((0, 0, 1, 2))):
        headroom = base - sum(
            d["amount_vnd"] for d in discounts if d["scope"] == "global_proportional"
        )
        if headroom <= 0:
            continue
        amount = headroom if rng.random() < 0.15 else rng.randint(1, headroom)
    ...
    return expense_dict(
        ...
        surcharges,
        discounts,
        fuzz_advancer(rng, participants),
    )
...
def corrupt(rng: random.Random, e: dict) -> None:
    ...
    if roll == 0:
        ...
        participants[rng.randrange(len(participants))] = rng.choice(
            (" a", "a ", "", "x" * 65, IDEOGRAPHIC_SPACE, "b\n")
        )
    elif roll == 5 and e["items"]:
        element = rng.choice(e["items"] + e["surcharges"] + e["discounts"])
        key = next(
            k
            for k in ("item_id", "surcharge_id", "discount_id")
            if k in element and (k != "item_id" or "shared_by" in element)
        )
        element[key] = rng.choice((" i0", "", "y" * 65, "i0", "i1", "total"))
    elif roll == 6 and e["surcharges"]:
        rng.choice(e["surcharges"])["kind"] = rng.choice(
            ("", "k" * 33, E_DOT_CIRCUMFLEX * 11, "k" * 32)
        )
    elif roll == 7 and e["surcharges"]:
        rng.choice(e["surcharges"])["mode"] = rng.choice(
            ("Even", "", "evenly", "proportional")
        )
        ...
        target["item_id"] = (
            ...
            else rng.choice([i["item_id"] for i in e["items"]] or ["i0"])
        )
    elif roll == 10 and e["items"]:
        target = rng.choice(e["items"])
        target["shared_by"] = rng.choice(
            (
                [],
                target["shared_by"] + target["shared_by"][:1],
                target["shared_by"] + ["stranger"],
            )
        )
        ...
        e["discounts"].append(
            discount("gx", rng.randint(1, 3 * MAX), "global_proportional")
        )
    else:
...
def apportion_edges() -> list[dict]:
    ...
    thirds = ["a", "b", "c"]
    out.append(
        apportion_case("thirds_advancer_b", 1, {p: Fraction(1, 3) for p in thirds}, "b")
    )
    out.append(
        apportion_case(
            "thirds_no_advancer", 2, {p: Fraction(2, 3) for p in thirds}, None
        )
    )
    out.append(
        apportion_case(
            "negative_thirds", -1, {p: Fraction(-1, 3) for p in thirds}, None
        )
    )
    out.append(
        apportion_case(
            "negative_two_thirds_advancer_c",
            -2,
            {p: Fraction(-2, 3) for p in thirds},
            "c",
        )
    )
    out.append(
        apportion_case(
            ...
            1,
            {"a": Fraction(-4, 3), "b": Fraction(7, 3)},
            None,
        )
    )
    out.append(
        apportion_case(
            "negative_integer_shares", -5, {"a": Fraction(-2), "b": Fraction(-3)}, "a"
        )
    )
    out.append(
        apportion_case("half_ties", 1, {"a": Fraction(1, 2), "b": Fraction(1, 2)}, None)
    )
    out.append(
        apportion_case(
            "half_ties_advancer_b", 1, {"a": Fraction(1, 2), "b": Fraction(1, 2)}, "b"
        )
    )
    out.append(
        apportion_case(
            "negative_half_ties", -1, {"a": Fraction(-1, 2), "b": Fraction(-1, 2)}, "b"
        )
    )
    out.append(
        apportion_case(
            "three_halves", 3, {"a": Fraction(3, 2), "b": Fraction(3, 2)}, None
        )
    )
    out.append(
        apportion_case(
            "minus_three_halves", -3, {"a": Fraction(-3, 2), "b": Fraction(-3, 2)}, None
        )
    )
    out.append(
        apportion_case(
            ...
            2,
            {
                GRINNING_FACE: Fraction(1, 2),
                CJK_MIDDLE: Fraction(1, 2),
                "Z": Fraction(1, 2),
                "a": Fraction(1, 2),
            },
            None,
    ...
    out.append(
        apportion_case(
            "past_int64",
            2**64 + 1,
            {"a": Fraction(2**64 + 1, 2), "b": Fraction(2**64 + 1, 2)},
            None,
        )
    )
    out.append(
...
def apportion_fuzz(seed: int, count: int) -> list[dict]:
    ...
    for i in range(count):
        ...
        fractions = [
            Fraction(
                rng.randint(-magnitude, magnitude),
                rng.choice((denominator, rng.randint(1, 13))),
            )
        ...
        out.append(
            apportion_case(
                f"fuzz/{i}", total, exact_summing_to(total, people, fractions), advancer
            )
        )
    return out
...
def bill_dict(
    participants, printed, items, surcharges=(), discounts=(), advancer=None
) -> dict:
    return {
        "participants": list(participants),
...
def billdraft_edges() -> list[dict]:
    ...
    out = [
        ...
        bill_case(
            "one_confirmed",
            bill_dict(people, 100, [bill_item("pho", 100, [("a", ok)])], advancer="a"),
        ),
        bill_case(
            "one_suggested",
            bill_dict(people, 100, [bill_item("pho", 100, [("a", ai)])]),
        ),
        bill_case(
            "mixed_item",
            bill_dict(people, 100, [bill_item("pho", 100, [("a", ok), ("b", ai)])]),
        ),
        bill_case("no_assignee", bill_dict(people, 100, [bill_item("pho", 100, [])])),
        bill_case(
            "no_assignee_beats_bad_source",
            bill_dict(
                people, 100, [bill_item("a", 1, [("a", "AI")]), bill_item("b", 1, [])]
            ),
        ),
        bill_case(
            "bad_source_later_key_still_found",
            bill_dict(
                people,
                100,
                [bill_item("z", 1, [("a", "")]), bill_item("a", 1, [("a", ok)])],
            ),
        ),
        bill_case(
            "source_with_space",
            bill_dict(people, 100, [bill_item("a", 1, [("a", "confirmed ")])]),
        ),
        bill_case(
            "source_upper",
            bill_dict(people, 100, [bill_item("a", 1, [("a", "CONFIRMED")])]),
        ...
        bill_case(
            ...
            bill_dict(
                ...
                [
                    bill_item(k, 10, [("a", ai)])
                    for k in ("b", "a", "Z", E_ACUTE, GRINNING_FACE, CJK_MIDDLE, "aa")
                ],
            ),
        ),
        bill_case(
            ...
            bill_dict(
                people,
                None,
                [bill_item("i", 100, [("a", ok)])],
                [surcharge("s", "vat", 8, "proportional")],
                [
                    discount("d", 30, "item", "i"),
                    discount("g", 5, "global_proportional"),
                ],
            ),
        ),
        bill_case(
            ...
            bill_dict(
                ...
                7,
                [bill_item("i", 100, [("a", ok)])],
                [surcharge("s", "vat", 8, "even")],
            ),
        ),
        bill_case(
            "printed_total_zero",
            bill_dict(people, 0, [bill_item("i", 100, [("a", ok)])]),
        ),
        bill_case(
            "listed_total_negative",
            bill_dict(
                people,
                None,
                [bill_item("i", 1, [("a", ok)])],
        ...
        bill_case(
            "listed_total_past_int64",
            bill_dict(
                people,
                None,
                [
                    bill_item("i", INT64_MAX, [("a", ok)]),
                    bill_item("j", INT64_MAX, [("b", ok)]),
                ],
            ),
        ),
        bill_case(
            "listed_total_below_int64",
            bill_dict(
                people,
                None,
                [bill_item("i", 1, [("a", ok)])],
                [],
                [
                    discount("g", INT64_MAX, "global_proportional"),
                    discount("h", INT64_MAX, "global_proportional"),
                ],
            ),
        ),
        bill_case(
            "negative_item_amount_passes_through",
            bill_dict(people, None, [bill_item("i", -5, [("a", ok)])]),
        ),
        bill_case(
            "duplicate_keys_pass_through",
            bill_dict(
                people,
                None,
                [bill_item("i", 5, [("a", ai)]), bill_item("i", 6, [("b", ai)])],
            ),
        ),
        bill_case(
            "stranger_is_left_for_the_allocator",
            bill_dict(people, None, [bill_item("i", 5, [("zz", ok)])]),
        ),
        bill_case(
            "no_participants", bill_dict([], None, [bill_item("i", 5, [("a", ok)])])
        ),
        bill_case(
            "repeated_share",
            bill_dict(people, None, [bill_item("i", 5, [("a", ok), ("a", ai)])]),
        ),
    ]
    return out
...
def billdraft_fuzz(seed: int, count: int) -> list[dict]:
    ...
    for i in range(count):
        ...
        for key in rng.sample(
            ENTITY_POOL + ("pho", "bia", "b", "a"), rng.choice((0, 1, 1, 2, 3, 5))
        ):
            shares = []
            for _ in range(rng.choice((0, 1, 1, 2, 3)) if rng.random() < 0.95 else 0):
                ...
                shares.append(
                    (
                        ...
                        rng.choice(sources)
                        if rng.random() < 0.3
                        else rng.choice(sources[:5]),
                    )
                )
            amount = (
                rng.choice((fuzz_amount(rng), 0, -1, INT64_MAX))
                if rng.random() < 0.1
                else fuzz_amount(rng)
            )
        ...
        surcharges = [
            surcharge(
                f"s{k}",
                rng.choice(KINDS),
                rng.choice((fuzz_amount(rng), INT64_MAX)),
                rng.choice(("even", "proportional")),
            )
        ...
        discounts = [
            ...
            for k in range(rng.choice((0, 0, 1)))
        ]
        printed = (
            None if rng.random() < 0.5 else rng.choice((0, fuzz_amount(rng), INT64_MAX))
        )
        ...
        out.append(
            bill_case(
                f"fuzz/{i}",
                bill_dict(
                    participants, printed, items, surcharges, discounts, advancer
                ),
            )
        )
    return out
...
def budget_edges() -> list[dict]:
    out = [
        ...
        budget_case(
            "zero_finished_headcount", [outing(headcount=0, split_total=0)], 0, 450_000
        ),
        budget_case(
            "zero_live_headcount",
            [
        ...
        budget_case(
            "over_budget",
            [outing(headcount=3, budget_vnd=100, split_total=301, in_progress=True)],
            3,
            None,
        ),
        budget_case(
            "exactly_on_budget",
            [outing(headcount=3, budget_vnd=100, split_total=300, in_progress=True)],
            3,
            None,
        ),
        budget_case(
            "floor_not_round",
            [outing(headcount=3, split_total=2), outing(headcount=3, split_total=2)],
            3,
            0,
        ),
        budget_case(
            "split_total_past_int64",
            [
                outing(headcount=3, split_total=2**64 + 5),
                outing(headcount=2, split_total=2**70, in_progress=True),
            ],
            3,
            INT64_MAX,
        ),
        budget_case(
            "candidate_int64_max",
            [outing(headcount=1, split_total=INT64_MAX)],
            1,
            INT64_MAX,
        ),
        budget_case(
            ...
            [
                outing(
                    title=IDEOGRAPHIC_SPACE + " Hội An\t" + NEXT_LINE, in_progress=True
                )
            ],
            1,
            None,
        ...
        budget_case(
            "title_zero_width_is_not_blank",
            [outing(title=ZERO_WIDTH_SPACE, in_progress=True)],
            1,
            None,
        ),
        budget_case("finished_title_stripped_nowhere", [outing(title=" x ")], 1, None),
    ]
    for candidate, _verdict in (
        (161_999, "re-hon"),
        (162_000, "nhu-thuong"),
        (198_000, "nhu-thuong"),
        (198_001, "cao-hon"),
        ...
        out.append(
            budget_case(
                f"band/{candidate}",
                [outing(headcount=3, split_total=540_000)],
                3,
                candidate,
            )
        )
    for field, value in (
        ...
        ("split_total_vnd", -1),
        ("split_total_vnd", None),
        ("split_total_vnd", -(2**64)),
        ("headcount", INT64_MIN),
    ):
        bad = outing()
        bad[field] = value
        ...
        out.append(
            budget_case(
                f"invalid_live/{field}/{len(out)}",
                [outing(), dict(bad, in_progress=True)],
                2,
                None,
            )
        )
    ...
    out += [
        ...
        budget_case(
            "invalid_after_valid_live",
            [outing(in_progress=True), outing(title="")],
            2,
            5,
        ),
    ]
...
def budget_fuzz(seed: int, count: int) -> list[dict]:
    ...
    for i in range(count):
        ...
        for k in range(rng.choice((0, 1, 2, 3, 5, 8))):
            ...
            outings.append(
                outing(
                    title=rng.choice(titles[:2])
                    if rng.random() < 0.97
                    else rng.choice(titles),
                    headcount=headcount if rng.random() < 0.98 else -1,
                    budget_vnd=rng.choice((0, 100_000, 300_000, rng.randint(0, 10**7)))
                    if rng.random() < 0.98
                    else -1,
                    split_total=split_total
                    if rng.random() < 0.98
                    else rng.choice((None, -1)),
...
ROLLUP_KINDS = (
    ...
    "Shipping",
    LONG_S + "hipping",
    "SHIPP" + DOTTED_CAPITAL_I + "NG",
    "shipp" + DOTTED_CAPITAL_I.lower() + "ng",
    "vat ",
    " vat",
    FULLWIDTH_V + "AT",
    KELVIN,
...
def expense_edges() -> list[dict]:
    out = [
        rollup_case(
            "test_pure_even_split_projects_total_to_subtotal",
            {"items": [], "surcharges": [], "discounts": [], "total_vnd": 82000},
        ),
        rollup_case(
        ...
        rollup_case(
            "only_a_discount_is_not_an_even_split",
            {
                "items": [],
                "surcharges": [],
                "discounts": [{"amount_vnd": 5}],
                "total_vnd": 0,
            },
        ),
        rollup_case(
            ...
            {
                "items": [],
                "surcharges": [{"kind": "vat", "amount_vnd": 5}],
                "discounts": [],
                "total_vnd": 5,
            },
        ),
        rollup_case(
            "sums_past_int64",
            {
                ...
                "surcharges": [{"kind": "VAT", "amount_vnd": INT64_MAX}] * 2
                + [{"kind": "x", "amount_vnd": INT64_MAX}] * 2,
    ...
    for k, kind in enumerate(ROLLUP_KINDS):
        out.append(
            rollup_case(
                ...
                {
                    "items": [{"amount_vnd": 1}],
                    "surcharges": [{"kind": kind, "amount_vnd": 7}],
                    "discounts": [],
                    "total_vnd": 8,
                },
            )
...
def expense_fuzz(seed: int, count: int) -> list[dict]:
    ...
    for i in range(count):
        e = {
            "items": [
                {"amount_vnd": rng.choice((fuzz_amount(rng), INT64_MAX))}
                for _ in range(rng.choice((0, 0, 1, 2, 4)))
            ],
            "surcharges": [
                {
                    "kind": rng.choice(ROLLUP_KINDS + KINDS),
                    "amount_vnd": rng.choice((fuzz_amount(rng), INT64_MAX)),
                }
                for _ in range(rng.choice((0, 0, 1, 2, 4)))
            ],
...
def obligations_of(rng: random.Random, most: int = 5) -> list[dict]:
    return [
        {
            "sender_id": rng.choice(("ha", "nam", "binh", "")),
...
def collection_edges() -> list[dict]:
        ...
        for event in EVENTS:
            ...
            for k, context in enumerate(contexts if legal else contexts[:2]):
                out.append(
                    case(
                        "transition",
                        f"transition/{state}/{event}/{k}",
                        {"state": state, "event": event, "context": context},
                    )
                )
    for k, context in enumerate(contexts[1:]):
        out.append(
            case("unmet_freeze_requirements", f"freeze/{k}", {"context": context})
        )
    ...
    lists = (
        ...
        [
            {"sender_id": "ha", "status": "over_confirmed"},
            {"sender_id": "nam", "status": "confirmed"},
        ],
        [
            {"sender_id": "ha", "status": "waived"},
        ...
        [
            {"sender_id": "ha", "status": "confirmed"},
            {"sender_id": "ha", "status": "waived"},
            {"sender_id": "nam", "status": "disputed"},
        ],
    )
    for k, obligations in enumerate(lists):
        out.append(
            case("terminal_state_for", f"terminal/{k}", {"obligations": obligations})
        )
    ...
    stamps = (
        ...
        datetime(2026, 9, 1, 9, 30, 12, 345678, tzinfo=UTC)
        ...
        for k, last in enumerate(stamps):
            out.append(
                case(
                    "is_stale",
                    f"stale/0/{j}/{k}",
                    {"now": base, "due_at": due, "last_meaningful_activity_at": last},
                )
            )
        ...
        for exposed in (None, EXPOSED):
            ...
            out.append(
                case(
                    ...
                    f"rate/{state}/{exposed is not None}",
                    {"batch": batch},
                )
            )
...
def collection_fuzz(seed: int, count: int) -> list[dict]:
    ...
    for i in range(transitions):
        ...
        out.append(
            case(
                "transition",
                f"fuzz/{i}",
                {"state": state, "event": event, "context": context_of(rng)},
            )
        )
    for i in range(max(1, terminal // 2)):
        obligations = obligations_of(rng, 6)
        out.append(
            case("terminal_state_for", f"fuzz/{i}", {"obligations": obligations})
        )
    ...
    for i in range(stale):
        ...
        def stamp():
            ...
            shift = timedelta(
                days=rng.choice((0, 7, 14, 21)),
                microseconds=rng.choice((0, 1, -1, rng.randint(-(10**11), 10**11))),
            )
        ...
        out.append(
            case(
                ...
                f"fuzz/{i}",
                {
                    "now": start.isoformat(),
                    "due_at": stamp(),
                    "last_meaningful_activity_at": stamp(),
                },
            )
        )
    return out
...
def scope_case(name: str, envelope: dict, obligations: list) -> dict:
    return case(
        "capability_scope", name, {"envelope": envelope, "obligations": obligations}
    )
...
def capability_edges() -> list[dict]:
    ...
    return [
        scope_case(
            "test_scope_is_one_sender_and_one_immutable_batch_version",
            env,
            [scoped("o2"), scoped("o1")],
        ),
        ...
        scope_case(
            "version_checked_before_sender",
            env,
            [scoped("o1", version="v2", sender="x")],
        ),
        scope_case(
            "first_crossing_wins",
            env,
            [scoped("o1", sender="x"), scoped("o2", version="v2")],
        ),
        scope_case("empty", env, []),
        scope_case("duplicate", env, [scoped("o1"), scoped("o1")]),
        scope_case(
            "crossing_after_duplicate",
            env,
            [scoped("o1"), scoped("o1"), scoped("o2", sender="x")],
        ),
        scope_case(
            "ids_sort_by_code_point",
            env,
            [
                scoped(GRINNING_FACE),
                scoped(CJK_MIDDLE),
                scoped(E_ACUTE),
                scoped("z"),
                scoped("Z"),
        ...
        scope_case(
            "uuid_ids",
            {"batch_version_id": C, "sender_id": A},
            [scoped(HIGH, C, A), scoped(LOW, C, A), scoped(F, C, A)],
        ),
        scope_case(
            "uuid_case_differs",
            {"batch_version_id": C, "sender_id": A},
            [scoped(LOW, C, A.upper())],
        ),
    ]
...
def capability_fuzz(seed: int, count: int) -> list[dict]:
    ...
    for i in range(count):
        envelope = {
            "batch_version_id": rng.choice(("v1", "v2")),
            "sender_id": rng.choice(("ha", "nam")),
        }
        obligations = [
            scoped(
                rng.choice(("o1", "o2", "o3", "O1", E_ACUTE, "o10")),
                rng.choice(("v1", "v1", "v1", "v2"))
                if rng.random() < 0.9
                else envelope["batch_version_id"],
                rng.choice(("ha", "ha", "nam")),
            )
    ...
    for k, receipts in enumerate(receipt_lists):
        out.append(
            case(
                "confirmed_total",
                f"total/{k}",
                {"receipt_confirmations": receipts_of(receipts)},
            )
        )
...
def ledger_status_fuzz(seed: int, count: int) -> list[dict]:
    ...
    for i in range(statuses):
        declared = (
            rng.choice(
                (rng.randint(1, 10**6), rng.randint(1, 9) * 1000, INT64_MAX, 0, -1)
            )
            if rng.random() < 0.97
            else INT64_MIN
        )
            ...
            if roll < 0.4 and declared > 0:
                amount = rng.choice(
                    (
                        declared,
                        declared - 1,
                        declared + 1 if declared < INT64_MAX else declared,
                        max(1, declared // 2),
                    )
                )
            elif roll < 0.95:
    ...
    for i in range(suggestions):
        names = rng.sample(
            ("a", "b", "c", "d", "e", "f", "g", "h", E_ACUTE, GRINNING_FACE),
            rng.randint(0, 8),
        )
        amounts = [rng.randint(-50, 50) for _ in names[:-1]]
        if names:
            amounts.append(
                -sum(amounts) + (rng.choice((0, 0, 0, 1)) if rng.random() < 0.3 else 0)
            )
...
class Stub:
    ...
    def list_members(self, context_id):
        ...
        return [
            SimpleNamespace(person_id=uuid.UUID(p), state=s)
            for p, s in self.facts["roster"]
        ]
    ...
    return schemas.ExpenseInput.model_construct(
        ...
        items=[
            schemas.ExpenseItemInput.model_construct(
                ...
                shared_by=[uuid.UUID(p) for p in i["shared_by"]],
            )
            for i in e["items"]
        ],
        surcharges=[
            schemas.ExpenseSurchargeInput.model_construct(**s) for s in e["surcharges"]
        ],
        discounts=[
            schemas.ExpenseDiscountInput.model_construct(**d) for d in e["discounts"]
        ],
    )
...
def run_propose(expense: dict) -> dict:
    ...
    return {
        "calls": stub.calls,
        "problem": None,
        "allocation": wire_allocation_shape(
            response.allocation.model_dump(mode="json")
        ),
    }
...
def run_confirm(
    ...
    try:
        new_service(stub).confirm_expense(
            uuid.UUID(F), request, actor(actor_id, actor_roles)
        )
        ...
        items=[
            schemas.BillItemCreateRequest.model_construct(
                ...
                suggested_participant_ids=[
                    uuid.UUID(p) for p in line["suggested_participant_ids"]
                ],
            )
...
def run_bill_assignment(items) -> dict:
    ...
    return {
        ...
        "suggested_item_keys": response.suggested_item_keys,
    }
...
def run_split(bill_spec, for_ledger, paid_by_id, roster) -> dict:
    ...
    request = schemas.BillSplitRequest.model_construct(
        for_ledger=for_ledger,
        paid_by_id=None if paid_by_id is None else uuid.UUID(paid_by_id),
    )
    outcome = {
        "calls": stub.calls,
        "problem": None,
    ...
    return BatchInputs(
        expenses=tuple(
            ConfirmedExpense(
                ...
                allocations=tuple(
                    AllocationRow(
                        id=uuid.UUID(r), participant_id=uuid.UUID(p), amount_vnd=a
                    )
                    for r, p, a in e["allocations"]
                ),
            )
            for e in spec["expenses"]
        ),
        unavailable_version_ids=tuple(
            uuid.UUID(v) for v in spec["unavailable_version_ids"]
        ),
    )
...
def run_create_batch(due_at, expense_version_ids, all_inputs, selected_inputs) -> dict:
    stub = Stub(
        all=None if all_inputs is None else batch_inputs(all_inputs),
        selected=batch_inputs(selected_inputs),
    )
    request = schemas.BatchCreateRequest.model_construct(
        context_id=uuid.UUID(CONTEXT),
        expense_version_ids=None
        if expense_version_ids is None
        else [uuid.UUID(v) for v in expense_version_ids],
        due_at=iso(due_at),
    )
    try:
        ...
        drafts = [
            {
                ...
                "source_expense_version_ids": [
                    str(v) for v in d.source_expense_version_ids
                ],
                "sources": [
                    [str(r.id), str(r.participant_id), r.amount_vnd] for r in d.sources
                ],
            }
...
def run_publish(batch, delivery_method, expires_at) -> dict:
    record = BatchForPublish(
        ...
        obligations=tuple(
            PublishObligation(
                id=uuid.UUID(o),
                batch_version_id=uuid.UUID(v),
                sender_id=uuid.UUID(s),
                recipient_id=uuid.UUID(r),
                amount_vnd=a,
            )
    ...
    request = schemas.BatchPublishRequest.model_construct(
        delivery_method=delivery_method, guest_link_expires_at=iso(expires_at)
    )
    try:
    ...
    links = [
        {
            ...
            "obligations": [
                [str(o.obligation_id), o.amount_vnd] for o in link.obligations
            ],
        }
...
def run_receipt(declared_amount_vnd, receipt_amounts_vnd) -> dict:
    stub = Stub(
        target=ReceiptTarget(
            obligation_id=uuid.UUID(D),
            recipient_id=uuid.UUID(A),
            amount_vnd=declared_amount_vnd,
        ),
        record=ReceiptRecord(
            id=uuid.UUID(E),
            obligation_id=uuid.UUID(D),
            amount_vnd=1000,
            receipt_amounts_vnd=tuple(receipt_amounts_vnd),
        ),
    )
    request = schemas.ReceiptConfirmationRequest.model_construct(
        amount_vnd=1000, idempotency_key=uuid.UUID(F), payment_report_id=None
    )
    try:
        response = new_service(stub).confirm_receipt(
            uuid.UUID(D), request, actor(A, ("recipient",))
        )
    except ApiProblem as exc:
...
def run_group_budget(outings, roster, candidate_per_person_vnd) -> dict:
    recap = [
        SimpleNamespace(
            outing=SimpleNamespace(
                id=uuid.UUID(o["outing_id"]),
                title=o["title"],
                headcount=o["headcount"],
                budget_per_person_vnd=o["budget_per_person_vnd"],
            ),
    ...
    response = new_service(stub).group_budget(
        uuid.UUID(CONTEXT), actor(), candidate_per_person_vnd=candidate_per_person_vnd
    )
...
def uuid_expense(rng: random.Random) -> dict:
    ...
    def to_uuid(name: str) -> str:
        if name not in mapping:
            mapping[name] = (
                PEOPLE[len(mapping)] if len(mapping) < len(PEOPLE) else random_uuid(rng)
            )
        return mapping[name]
    ...
    e["advancer_id"] = (
        mapping[advancer]
        if advancer in mapping
        else rng.choice((F, HIGH, random_uuid(rng)))
    )
    if rng.random() < 0.45:
        ...
        if roll == 0:
            ...
            rng.choice(e["items"])["shared_by"] = rng.choice(
                ([], [random_uuid(rng)], [e["participants"][0]] * 2)
            )
...
def roster_of(rng: random.Random, ids) -> list[list[str]]:
    ...
    if roll < 0.15 and roster:
        ...
        roster[rng.randrange(len(roster))][1] = rng.choice(
            ("invited", "left", "ACTIVE", "")
        )
    elif roll < 0.3:
...
def confirm_case(
    name, e, roster, recorded_by, expected, acknowledge, actor_id=A, roles=("member",)
) -> dict:
    args = {
        "actor_id": actor_id,
...
def create_bill_case(name, items_total, lines, roster) -> dict:
    return case(
        "create_bill",
        name,
        {"items_total_vnd": items_total, "lines": lines, "roster": roster},
    )
...
def publish_case(name, batch, delivery_method, expires_at) -> dict:
    return case(
        "publish_batch",
        name,
        {"batch": batch, "delivery_method": delivery_method, "expires_at": expires_at},
    )
...
def money_steps_edges() -> list[dict]:
    ...
    out += [
        ...
        propose_case(
            "uuid_byte_order", expense_dict([HIGH, LOW, F, A], 7, advancer=HIGH)
        ),
    ]
    ...
    out += [
        ...
        confirm_case(
            ...
            True,
            actor_id=B,
            roles=("member", "advancer"),
        ),
        confirm_case(
            "acknowledge_without_role",
            dinner,
            active(A, B, C),
            B,
            right,
        ...
        confirm_case(
            "stranger_payer_and_recorder",
            expense_dict([A, B], 10, advancer=HIGH),
            active(A, B),
            LOW,
            [[A, 5], [B, 5]],
            False,
        ...
        confirm_case(
            "invited_is_not_a_member",
            dinner,
            [[A, "active"], [B, "active"], [C, "invited"]],
            A,
            right,
            False,
        ),
        confirm_case(
            "strangers_before_acknowledgement",
            dinner,
            active(A),
            A,
            right,
        ...
        confirm_case(
            "proposal_changed_value",
            dinner,
            active(A, B, C),
            A,
            [[A, 33334], [B, 33333], [C, 33333]],
            False,
        ),
        confirm_case(
            "proposal_changed_missing", dinner, active(A, B, C), A, right[:2], False
        ),
        confirm_case(
            "proposal_changed_past_int64",
            dinner,
            active(A, B, C),
            A,
            [[A, 2**64], [B, 33334], [C, 33333]],
            False,
        ),
        confirm_case(
            "allocation_refused_before_compare",
            expense_dict([A, B], -5, advancer=A),
            active(A, B),
            A,
            [],
            False,
        ...
        confirm_case(
            ...
            expense_dict(
                [A, B],
                160000,
                [item("i", 100000, [A]), item("j", 50000, [A, B])],
                [
                    surcharge("s", "VAT", 12000, "proportional"),
                    surcharge("t", LONG_S + "hipping", 5000, "even"),
                    surcharge("u", "service", 3000, "even"),
                ],
                [discount("d", 10000, "global_proportional")],
                A,
            ),
            active(A, B),
            A,
    ...
    out += [
        create_bill_case(
            ...
            300,
            [
                {"line_total_vnd": 100, "suggested_participant_ids": [A]},
                {"line_total_vnd": 200, "suggested_participant_ids": []},
            ],
            active(A),
        ),
        create_bill_case(
        ...
        create_bill_case(
            "stranger_beats_mismatch",
            1,
            [{"line_total_vnd": 100, "suggested_participant_ids": [HIGH, LOW, HIGH]}],
            active(A),
        ),
        ...
        create_bill_case(
            "sum_past_int64",
            INT64_MAX,
            [{"line_total_vnd": INT64_MAX, "suggested_participant_ids": []}] * 2,
            [],
        ),
    ]
    for k, sources in enumerate(
        (
            [],
            [["confirmed"]],
            [["ai_suggested"]],
            [["confirmed"], []],
            [["confirmed", "ai_suggested"]],
            [["confirmed", "confirmed"], ["confirmed"]],
        )
    ):
        items = [
            {"item_key": key, "sources": item_sources}
            for key, item_sources in zip(("b", "a", E_ACUTE), sources, strict=False)
        ]
        out.append(case("bill_assignment", f"assignment/{k}", {"items": items}))
    out.append(
        case(
            "bill_assignment",
            "assignment/keys_in_byte_order",
            {
                "items": [
                    {"item_key": key, "sources": ["ai_suggested"]}
                    for key in ("b", "Z", GRINNING_FACE, CJK_MIDDLE, "a")
                ]
            },
        )
    )
    ...
    pho = {
        ...
        "shares": [[A, "confirmed"], [B, "confirmed"]],
    }
    bill_spec = {
    ...
    out += [
        ...
        split_case(
            "suggested_for_ledger",
            dict(bill_spec, items=[dict(pho, shares=[[A, "ai_suggested"]])]),
            True,
            A,
            active(A, B),
        ),
        ...
        split_case(
            "unknown_source",
            dict(bill_spec, items=[dict(pho, shares=[[A, "AI"]])]),
            False,
            A,
            active(A),
        ),
        split_case(
            "share_of_a_stranger",
            dict(bill_spec, items=[dict(pho, shares=[[HIGH, "confirmed"]])]),
            False,
            A,
            active(A),
        ),
        ...
        split_case(
            ...
            dict(bill_spec, printed_total_vnd=100000),
            False,
            A,
            active(A, B),
        ),
        split_case(
            "both_active_and_invited",
            bill_spec,
            False,
            A,
            [[A, "active"], [B, "active"], [B, "invited"]],
        ),
        split_case(
    ...
    def row(k):
        return str(uuid.UUID(int=(k + 1) * 0x9E3779B97F4A7C15F39CC0605CEDC835 % 2**128))
        ...
        "expenses": [
            {
                "version_id": v1,
                "paid_by_id": A,
                "allocations": [[row(0), A, 50], [row(1), B, 30], [row(2), C, 20]],
            }
        ],
    ...
    out += [
        ...
        batch_case(
            ...
            [v1, v2],
            None,
            dict(one, unavailable_version_ids=[v2]),
        ),
        batch_case(
            "unavailable_ignored_when_not_named",
            LATER,
            None,
            one,
            dict(one, unavailable_version_ids=[v2]),
        ...
        batch_case(
            ...
            LATER,
            None,
            {"expenses": [], "unavailable_version_ids": []},
            {"expenses": [], "unavailable_version_ids": []},
        ),
        batch_case(
            "named_empty_list",
            ...
            one,
            {
                "expenses": [
                    ...
                    },
                    {
                        "version_id": v1,
                        "paid_by_id": A,
                        "allocations": [[row(6), C, INT64_MAX], [row(7), B, 5]],
                    },
                    {
                        "version_id": v3,
                        "paid_by_id": B,
                        "allocations": [[row(8), A, 7]],
                    },
                ],
                "unavailable_version_ids": [],
            },
    ...
    version = D
    obligations = [
        [row(10), version, B, A, 30],
        [row(11), version, C, A, 20],
        [row(12), version, B, C, 5],
    ]
    batch = {
        "version_id": version,
        "status": "frozen",
    ...
    out += [
        ...
        publish_case(
            "senders_in_uuid_order",
            dict(
                batch,
                obligations=[
                    [row(13), version, HIGH, A, 1],
                    [row(14), version, LOW, A, 2],
                    [row(15), version, HIGH, B, 3],
                ],
            ),
            "personal_link",
            LATER,
        ...
        publish_case(
            "expired_beats_gates",
            dict(batch, advancer_acknowledged=False),
            "",
            NOW.isoformat(),
        ),
        publish_case(
            "not_acknowledged", dict(batch, advancer_acknowledged=False), "", LATER
        ),
        publish_case("no_delivery_method", batch, "", LATER),
        publish_case(
            "already_published", dict(batch, status="published"), "personal_link", LATER
        ),
        publish_case(
            "unknown_status", dict(batch, status="delivered"), "personal_link", LATER
        ),
        publish_case(
            "crosses_version",
            dict(batch, obligations=obligations + [[row(16), E, C, B, 4]]),
            "personal_link",
            LATER,
        ),
        publish_case(
            "duplicate_obligation",
            dict(batch, obligations=obligations + [obligations[0]]),
            "personal_link",
            LATER,
        ),
        publish_case(
            "no_obligations", dict(batch, obligations=[]), "personal_link", LATER
        ),
    ]
    for k, (declared, receipts) in enumerate(
        (
            (100, ()),
            (100, (40,)),
            (100, (40, 60)),
        ...
        out.append(
            case(
                ...
                {
                    "declared_amount_vnd": declared,
                    "receipt_amounts_vnd": list(receipts),
                },
            )
        )
    for k, (me, person) in enumerate(((A, A), (A, B), (HIGH, LOW))):
        out.append(
            case(
                "person_finance", f"finance/{k}", {"actor_id": me, "person_id": person}
            )
        )
    trips = [
        ...
        dict(
            outing(
                title=" Biển ", headcount=3, split_total=1_000_001, in_progress=True
            ),
            outing_id=B,
        ),
    ]
    out += [
        case(
            ...
            {
                "outings": trips,
                "roster": [[A, "active"], [B, "invited"], [C, "active"]],
        ...
        case(
            ...
            "budget/invalid",
            {
                "outings": [dict(outing(title=" "), outing_id=A)],
                "roster": [],
        ...
        case(
            ...
            "budget/past_int64",
            {
                "outings": [dict(outing(split_total=2**66), outing_id=A)],
                "roster": active(A),
...
def batch_fuzz_case(rng: random.Random, i: int) -> dict:
    ...
    for version in versions:
        ...
        for person in rng.sample(people, rng.randint(0, len(people))) + (
            [rng.choice(people)] if rng.random() < 0.1 else []
        ):
        ...
        expenses.append(
            {"version_id": version, "paid_by_id": payer, "allocations": rows}
        )
    inputs = {
        "expenses": expenses,
        "unavailable_version_ids": [random_uuid(rng)] if rng.random() < 0.15 else [],
    }
    named = rng.random() < 0.5
    due = rng.choice(
        (LATER, LATER, LATER, NOW.isoformat(), (NOW - timedelta(days=1)).isoformat())
    )
    return batch_case(
        f"fuzz/{i}", due, versions if named else None, None if named else inputs, inputs
    )
...
def money_steps_fuzz(seed: int, count: int) -> list[dict]:
    ...
    (
        ...
        receipts,
        budgets,
    ) = split_count(count, (800, 1200, 600, 600, 1500, 1200, 800, 600, 600))
    out = []
    ...
    for i in range(confirmations):
        ...
        roles = rng.choice(
            (("member",), ("member", "advancer"), ("member", "advancer", "recipient"))
        )
    ...
    for i in range(bills):
        lines = [
            {
                "line_total_vnd": rng.choice((fuzz_amount(rng), INT64_MAX)),
                "suggested_participant_ids": rng.sample(
                    PEOPLE, rng.choice((0, 0, 1, 2))
                ),
            }
        ...
        items_total = (
            min(INT64_MAX, max(0, total + rng.choice((0, 0, 0, 1, -1))))
            if total <= INT64_MAX
            else rng.choice((0, INT64_MAX))
        )
        ...
        items = [
            {
                "item_key": key,
                "sources": [
                    rng.choice(("confirmed", "confirmed", "ai_suggested"))
    ...
    for i in range(splits):
        ...
        items = [
            {
                "item_key": key,
                "line_total_vnd": rng.choice((fuzz_amount(rng), 0, -1))
                if rng.random() < 0.05
                else fuzz_amount(rng),
                "shares": [
                    [
                        rng.choice(members + [HIGH]) if members else HIGH,
                        rng.choice(sources),
                    ]
                    for _ in range(
                        rng.choice((0, 1, 1, 2, 3)) if rng.random() < 0.97 else 0
                    )
                ],
            }
        ...
        surcharges = [
            {
                ...
                "amount_vnd": fuzz_amount(rng),
                "mode": rng.choice(("even", "proportional")),
            }
            for k in range(rng.choice((0, 0, 1)))
        ]
        discounts = (
        ...
        listed = (
            sum(x["line_total_vnd"] for x in items)
            + sum(s["amount_vnd"] for s in surcharges)
            - sum(d["amount_vnd"] for d in discounts)
        )
        printed = (
            None if rng.random() < 0.5 else rng.choice((listed, listed, listed + 1, 0))
        )
        spec = {
            "printed_total_vnd": printed,
            "items": items,
        ...
        out.append(
            split_case(
                ...
                spec,
                rng.random() < 0.5,
                rng.choice(members + [None, HIGH]),
                roster_of(rng, members),
            )
        )
    ...
    for i in range(publications):
        ...
        obligations = [
            [
                random_uuid(rng),
                version if rng.random() < 0.97 else random_uuid(rng),
                rng.choice(senders),
                rng.choice(PEOPLE),
                rng.randint(1, 10**6),
            ]
        ...
        batch = {
            "version_id": version,
            "status": rng.choice(
                ("frozen", "frozen", "frozen", "accruing", "published", "")
            ),
        ...
        out.append(
            publish_case(
                f"fuzz/{i}",
                batch,
                rng.choice(("personal_link", "personal_link", "")),
                rng.choice((LATER, LATER, NOW.isoformat())),
            )
        )
    for i in range(receipts):
        ...
        receipts = [
            rng.choice(
                (rng.randint(1, 10**6), declared if declared > 0 else 1, INT64_MAX, 0)
            )
        ...
        out.append(
            case(
                "confirm_receipt",
                f"fuzz/{i}",
                {"declared_amount_vnd": declared, "receipt_amounts_vnd": receipts},
            )
        )
    for i in range(budgets):
        ...
        roster = [
            [random_uuid(rng), rng.choice(("active", "active", "invited", "left"))]
            for _ in range(rng.randint(0, 6))
        ]
        candidate = (
            None
            if rng.random() < 0.3
            else rng.choice((0, rng.randint(0, 10**6), INT64_MAX))
        )
        out.append(
            case(
                "group_budget",
...
FUNCTIONS = {
    ...
    "build_group_budget": lambda outings,
    active_member_count,
    candidate_per_person_vnd: budget.build_group_budget(
        outings,
    ...
    "is_stale": lambda now, due_at, last_meaningful_activity_at: collection.is_stale(
        iso(now), iso(due_at), iso(last_meaningful_activity_at)
    ),
    ...
    "split_bill": lambda **kw: run_split(
        kw["bill"], kw["for_ledger"], kw["paid_by_id"], kw["roster"]
    ),
    "create_batch": lambda **kw: run_create_batch(
        kw["due_at"], kw["expense_version_ids"], kw["all"], kw["selected"]
    ),
...
MODULES = {
    ...
    "money_steps": (
        "internal/domain/moneysteps",
        api_service,
        money_steps_constants,
        money_steps_edges,
        money_steps_fuzz,
        300,
        7900,
